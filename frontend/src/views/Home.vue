	...
	<v-container fluid class="pa-0 main-container">
		...
		<div class="content">
			<div :class="['title', titleClass]">
				...
				<v-btn class="tertiary accent--text" :x-large="$vuetify.breakpoint.mdAndUp"
					:large="$vuetify.breakpoint.smAndDown" to="/login">Accéder au forum</v-btn>
			</div>
			...
			<!-- Horaires et liens -->
			<v-row class="mx-7">
				<v-col class="col-12 col-sm-6" v-if="heure">
					<div class="text-h5 text-md-h4">Horaires</div>
					<div :class="['text-body-2 text-md-body-1 text-lg-h6', listSpacingClass]">
						<ul>
							<li>
								<div>{{ heure }}</div>
							</li>
				...
				<v-col class="col-12 col-sm-6 my-2">
					...
					<div :class="['text-body-2 text-md-body-1 text-lg-h6', listSpacingClass]">
						<ul>
							<li>
								La formation en
								<a class="tertiary--text" href="https://polytech.univ-lyon1.fr/formation/alternance"
							...
							<li>
								Informations
								<a class="tertiary--text" href="https://polytech.univ-lyon1.fr/etudiant" target="_blank"
...
export default {
	...
	created() {
		...
		this.updateHeure(this.forumHeure)
	},
	...
	methods: {
		...
		// Met à jour l'heure sur le template
		updateHeure(newHeure) {

			if (newHeure) {
				const heureSplited = newHeure.split(" - ");
				this.heure = `De ${heureSplited[0].replace(":", "h")} à ${heureSplited[1].replace(":", "h")}`;
			}
		},
	},
